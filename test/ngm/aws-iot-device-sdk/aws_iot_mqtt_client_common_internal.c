...
static IoT_Error_t _aws_iot_mqtt_internal_deliver_message(AWS_IoT_Client *pClient, char *pTopicName,
	...
	for(itr = 0; itr < AWS_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS; ++itr) {
		if(0 != pClient->clientData.messageHandlers[itr].topicName[0]) {
			IOT_INFO ("strcmp: buf(%s) readed(%.*s)", pClient->clientData.messageHandlers[itr].topicName, topicNameLen, pTopicName);
