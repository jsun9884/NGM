...
int
nmea_gpgll_init(nmea_parser_s *parser)
{
...
int
nmea_gpgll_allocate_data(nmea_parser_s *parser)
{
...
int
nmea_gpgll_set_default(nmea_parser_s *parser)
{
...
int
nmea_gpgll_free_data(nmea_s *data)
{
	free(data);
	return 0;
}
...
int
nmea_gpgll_parse(nmea_parser_s *parser, char *value, int val_index)
{
