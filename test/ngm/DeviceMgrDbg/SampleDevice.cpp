...
void SampleDevice::ApplyNativeParameter(ParameterPtr param)
{
	...
	if (name == "regularMsgTransmissionRate") {
		...
		log::info() << "ApplyNativeParameter: regularMsgTransmissionRate changed to " << m_regularMsgTransmissionRate;
	}
	else if (name == "autoHrEnabled") {
		m_autoHrEnabled = param->GetValue();
		
		log::info() << "ApplyNativeParameter: autoHrEnabled changed to " << (m_autoHrEnabled ? "true" : "false");
	}
	else {
