...
(function(window, angular, undefined) {
    ...
    .factory('ssSideNav', [
        ...
        function(
            ...
            $rootScope.theState = $state;


            var matchPage = function(section, page, newState) {
                var toState = newState ? newState.theState.current : null;

                if (!toState) {
                    return console.warn('ss-sidenav: `toState` key not found');
                }

                //console.log(self.includes(toState));
/*
                $scope.isExpanded = function(section) {
                    return !!section.pages.find(function(section_page){
                        return $state.includes(section_page.state);
                        //return ssSideNav.isPageSelected(section_page.state)
                    });
                };
*/
                if (!!self) {
                    if(section.pages.find(function(page){
                        return newState.theState.includes(page.state)
                    })){
            ...
            var onStateChangeStart = function(event, theState, toParams) {
                var newState = {
                    theState: theState,
                    toParams: toParams
                };
...
*/
            ...
            onStateChangeStart(null, $state, $stateParams);
    ...
    .controller('sidenavMainCtrl', [
        ...
        function(
            ...
            $scope.$watch("isOpen()", function(newValue, oldValue){
                $rootScope.shouldLockOpen = newValue;
                $scope.$parent.showTitles = newValue;
            },true);

            $scope.toggleLeft = function (page) {
...
//                $mdSidenav('left').close();
    ...
    .run(['$templateCache', function($templateCache) {
        $templateCache.put('views/ss/menu-link.tmpl.html',
            '<md-button\n' +
            //'   ss-style-color="{\'background-color\': (isSelected(section.state) || $state.includes(section.state)) ? \'light-blue.600\': \'primary.800\'}"' +
            '   ss-style-color="{\'background-color\': ($state.includes(section.state)) ? \'light-blue.600\': \'primary.800\'}"' +
            '   class="md-raised md-primary"' +
            '   ui-sref="{{section.state}}"\n' +
            '   ng-click="focusSection(section)">\n' +
            '   <md-icon ng-if="section.icon" class="section-icon" ng-class="{\'active\': (isSelected(section.state) || $state.includes(section.state))}">{{section.icon}}</md-icon><span class="item-title">{{section.name}}</span>\n' +
            '   <md-tooltip md-direction="top" class="sidebar-tooltip">{{section.name}}</md-tooltip>\n' +
        ...
        $templateCache.put('views/ss/menu-toggle.tmpl.html',
            ...
            '   <div flex layout="row">\n' +
            '       <md-icon ng-if="section.icon" class="section-icon" style="color: rgb(3, 155, 229)">{{section.icon}}</md-icon>\n' +
            '       <span class="item-title">{{section.name}}</span>\n' +
            '       <md-tooltip md-direction="top" class="sidebar-tooltip">{{section.name}}</md-tooltip>\n' +
            '       <span flex></span>\n' +
            '       <span aria-hidden="true" class="md-toggle-icon"\n' +
            '           ng-class="{\'toggled\' : isOpen(section)}">\n' +
            '           <md-icon class="toggle-icon" md-svg-src="md-toggle-arrow"></md-icon>\n' +
            '       </span>\n' +
            '   </div>\n' +
            '   <span class="md-visually-hidden">\n' +
        ...
        $templateCache.put('views/ss/menu-sidenav.tmpl.html',
            ...
            '        <h3 ss-style-color="{\'color\': \'primary.A300\'}" class="menu-heading md-subhead" ng-if="section.type === \'heading\'">' +
            '           <span class="item-title">{{shouldLockOpen}}{{section.name}}</span>\n' +
            '           <md-tooltip class="sidebar-tooltip" md-direction="top">{{section.name}}</md-tooltip>\n' +
            '        </h3>' +
            '        <menu-link section="section" ng-if="section.type === \'link\'"></menu-link>' +
            '        <menu-toggle section="section" ng-if="section.type === \'toggle\'"></menu-toggle>' +
            '        <ul ng-if="section.children">' +
            '            <li ng-repeat="child in section.children" ng-if="!child.hidden" ss-style-color="{\'background-color\': \'primary.800\'}"">' +
